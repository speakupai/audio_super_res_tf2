      ...
      for line in f:
        try:
          ...
          if(args.speaker == 'single'):
            upsample_wav('../data/vctk/VCTK-Corpus/wav48/p225/'
                         +line.strip(), args, model)
          else:
            upsample_wav('../data/vctk/VCTK-Corpus/'
                         +line.strip(), args, model)
        except EOFError:
...
def get_model(args, n_dim, r, from_ckpt=False, train=True, grocery='false'):
  ...
  print(args.model)
  # create model
  if args.model == 'audiotfilm':
    model = AudioTfilm(from_ckpt=from_ckpt, n_dim=n_dim, r=r,
                              pool_size = args.pool_size, strides=args.strides,
